...
function buildPackageJson(name: string, stackIds: string[]): string {
  ...
  return JSON.stringify(
    {
      name: name.toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-|-$/g, "") || "prism-starter",
      version: "0.1.0",
      private: true,
      scripts: {
        dev: "next dev",
...
Generated by [Prism Studio](https://prism.studio).
`
}
...
export async function downloadStarterZip(input: ExportInput): Promise<void> {
  ...
  const slug = input.theme.name.toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-|-$/g, "") || "prism-starter"
